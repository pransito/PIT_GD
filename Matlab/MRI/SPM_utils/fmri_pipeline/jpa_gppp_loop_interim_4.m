...
%ss_level.gen.sjinfo.path = 'E:\Google Drive\Promotion\VPPG\VPPG_Exchange\Experimente\PDT\Daten\pilot\sjinfo.mat';
...
includeList = all_subs(1:length(all_subs)); 
ss_level.gen.includeList = includeList;                                % default: {''},i.e. all IDs will be part of the test
...
ss_level.gppi.FirstLVLModel={'MRT\NIFTI\PDT\results\PDT_ss_design_DEZ_sm_orth_val'};
% should the sequence of subjects be permuted (useful if running multiple
% matlabs, each doing a different VOI)
ss_level.gppi.permute_subs = 1;
...
base_path_ROIs = 'C:\Users\genaucka\Google Drive\Library\MATLAB\PDT\MRI\sl\ROIs\from_ext_HD\ROIs\ROIs_gPPI_seeds';
VOI_folder = 'C:\Users\genaucka\Google Drive\Library\MATLAB\PDT\MRI\sl\ROIs\from_ext_HD\ROIs\ROIs_gPPI_seeds';
cd(VOI_folder);
...
%     end
    ...
    for modelIt=1:1:length(ss_level.gppi.FirstLVLModel)
        ...
        for roiIt=1:1:length(ss_level.gppi.VOI)
            ...
            if (ss_level.gppi.permute_subs)
                ind_sequence = ind_sequence(randperm(length(ind_sequence)));
            end
            for indIt=ind_sequence
                if (indSearchIn(indIt) ~= 0) % then do test
                    P.subject = ids{indSearchIn(indIt)};
                    P.directory = match{indIt};
                     % read Contrasts from SPM
                    if ss_level.gppi.readSPMContrasts
                        try
                            clear('SPM')
                            load([match{indIt} filesep 'SPM.mat']);
                            P.SPMContrasts = jpa_readContrasts(SPM , ss_level.gppi.minEvents);
                        catch ME
                            disp(ME.identifier);
                            cd(base_dir_lib)
                            rethrow(ME); 
                    ...
                    if ss_level.gppi.readScriptContrasts
                        P.ScriptContrasts = jpa_getGppiContrasts(ss_level.gppi.Contrasts);
                    end
                    % combine Contrasts
                    if ss_level.gppi.readSPMContrasts && ss_level.gppi.readScriptContrasts
                        P.Contrasts = [P.SPMContrasts P.ScriptContrasts];
                    elseif ~ss_level.gppi.readSPMContrasts && ss_level.gppi.readScriptContrasts
                        P.Contrasts = P.ScriptContrasts;
                    elseif ss_level.gppi.readSPMContrasts && ~ss_level.gppi.readScriptContrasts
                        P.Contrasts = P.SPMContrasts;
                    else
                        error('No contrasts defined!')
                    end
                    % run PPPI
                    cd(P.directory)
                    tmp = PPPI_checkstruct(P);
