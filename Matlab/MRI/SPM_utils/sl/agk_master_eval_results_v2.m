...
function all_sig_results = agk_master_eval_results_v2(pSPM,ROI,reference,cur_atlas,thr_p,thr_k)
...
for ii = 1:length(pSPM)
    ...
    try
        ...
        try
            ...
            warning('No cons in the SPM. Probably SPM was never estimated. I''ll continue with next SPM.')
            continue
        end
        ...
        for kk = 1:cur_numcon
            ...
            try
                ...
                disp('SVC did not work')
                TabDat.results{ii,kk} = lasterror;
            end
        end

    catch lasterror
        disp('AN ERROR OCCURED NOSPM NO TABDAT!')
        rethrow(lasterror)
        TabDat.coninfo{ii,1} = 'NoSPM';
        TabDat.SPMname{ii,1} = 'NoSPM';
        TabDat.results{ii,1} = 'NoSPM';
    end
end
    ...
    for ii = 1:size(TabDat.results,1)
        ...
        for jj = 1:size(TabDat.results,2)
            ...
            try
                ...
                disp('Cannnot threshold results, because there is no SPM, so no results.')
            end
        end
    end
