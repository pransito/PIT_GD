...
% 4 task on: PIC, PIC+GAM, FEEDBACK, ITI on
% 3 param modulators in PIC: category dummy coded (gam, neg, neu)
% 3 param mod in PIC+GAM: gain, abs(loss), ed
...
function error_message = agk_make_PDT_ss_pspml_design(cur_sub, ...
...
% try
    ...
    if acc_rec == 0
        names       = {'ITI','Pic.on','Pic.gam.opt.on','feedback'};
        onsets      = cell(1,4);
        durations   = cell(1,4);
    end
...
%     try
        for ii = 1 : length(P.cur.choice)
            ...
            % ITI
            onsets(1)    = {[cell2mat(onsets(1)) P.t.cur_trial.iti_on(ii) + P.t.start_exp - phys0]};
            if ~isempty (fixed_dur)
                durations(1) = {[cell2mat(durations(1)) fixed_dur]};
            else
                durations(1) = {[cell2mat(durations(1)) (P.t.cur_trial.stim_on(ii) - P.t.cur_trial.iti_on(ii))]};
            end
            % PIC ON
            onsets(2)    = {[cell2mat(onsets(2)) P.t.cur_trial.stim_on(ii) + P.t.start_exp - phys0]};
            if ~isempty (fixed_dur)
                durations(2) = {[cell2mat(durations(2)) fixed_dur]};
            else
                durations(2) = {[cell2mat(durations(2)) (P.t.cur_trial.stimLA_on(ii) - P.t.cur_trial.stim_on(ii))]};
            end
            switch P.cur.cat(ii)
                case 1
                    cur_gam = 1;
            ...
            onsets(3)    = {[cell2mat(onsets(3)) P.t.cur_trial.stimLA_on(ii) + P.t.start_exp - phys0]};
            if ~isempty (fixed_dur)
                durations(3) = {[cell2mat(durations(3)) fixed_dur]};
            else
                durations(3) = {[cell2mat(durations(3)) (P.t.cur_trial.stimLA_off(ii) - P.t.cur_trial.stimLA_on(ii))]};
            end
            % gain
            ...
            % FEEDBACK NO PARAM MOD
            onsets(4)    = {[cell2mat(onsets(4)) P.t.cur_trial.stimLA_off(ii) + P.t.start_exp - phys0]};
            if ~isempty (fixed_dur)
                durations(4) = {[cell2mat(durations(4)) fixed_dur]};
            else
