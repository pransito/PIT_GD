...
% 2 task on: PIC, PIC+GAM, FEEDBACK
% 3 param modulators in PIC: category dummy coded (gam, neg, neu)
% 6 param mod in PIC+GAM: cat dummy coded, gain, abs(loss), ed
...
function error_message = agk_make_PDT_ss_pspml_design_edfix_03(cur_sub, ...
    aggr,do_center,run_it,acc_rec,fixed_dur)
% try
    %% PREPARATIONS
    % name of this analysis determines the results directory
    results_ssdir = 'pspml_edfix_03';
    ...
    if acc_rec == 0
        names       = {'Pic.on','Pic.gam.opt.on','feedback'};
        onsets      = cell(1,3);
        durations   = cell(1,3);
    end
...
%     try
        for ii = 1 : length(P.cur.choice)
            ...
            onsets(3)    = {[cell2mat(onsets(3)) P.t.cur_trial.stimLA_off(ii) + P.t.start_exp - phys0]};
            if ~isempty (fixed_dur)
                durations(3) = {[cell2mat(durations(3)) fixed_dur]};
            else
                durations(3) = {[cell2mat(durations(3)) P.instr.slow.time]};
            end
        end
