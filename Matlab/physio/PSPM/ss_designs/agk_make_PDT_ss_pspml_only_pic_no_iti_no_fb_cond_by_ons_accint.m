...
function error_message = agk_make_PDT_ss_pspml_only_pic_no_iti_no_fb_cond_by_ons_accint(cur_sub, ...
...
% ed_stps_on = [];
% for kk = 1:length(edabs_steps)
%     cur_name                         = ['Pic.Gam.opt.edabs' ...
...
for ii = 1 : length(P.cur.choice)
    ...
    % pmod for PIC ON (acceptance, dichotomized and mean centered)
    cur_acc = P.cur.choice(ii);
    cur_acc = agk_recode(P.cur.choice(ii),[1,2,3,4,5],[1,1,0,0,5]);
    cur_acc = cur_acc - mean_acc;
    ps{ons_code} = [ps{ons_code} cur_acc];
    
    % PIC PLUS GAMBLE ON (WITH CHOICE OPTIONS) (GAIN,LOSS,EDABS)
    onsets_to_be_filled = [];
...
%     onsets_to_be_filled(3) = cur_ed_on;
    ...
    for jj = 1:length(onsets_to_be_filled)
        if jj == 1
            % just to see gain and loss separately
            cur_offset = 0.25;
        else
            cur_offset = 0.20;
        end
