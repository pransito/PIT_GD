...
function error_message = agk_make_PDT_ss_pspml_only_pic_no_iti_no_fb_cond_by_ons_accint3(cur_sub, ...
...
names       = {'Pic.gam','Pic.neg','Pic.pos',...
               'Gam.opt.gam.acc','Gam.opt.neg.acc','Gam.opt.pos.acc'};
...
for ii = 1 : length(P.cur.choice)
    ...
    if ~isnan(ons_code)
        % PIC PHASE
        onsets(ons_code(1)) = {[cell2mat(onsets(ons_code(1))) P.t.cur_trial.stim_on(ii) + P.t.start_exp - phys0]};
        if ~isempty (fixed_dur)
            durations(ons_code(1)) = {[cell2mat(durations(ons_code(1))) fixed_dur]};
        else
            durations(ons_code(1)) = {[cell2mat(durations(ons_code(1))) (P.t.cur_trial.stimLA_on(ii) - P.t.cur_trial.stim_on(ii))]};
        end
    end
