...
function pic_scr_feedback = agk_get_pic_mri_timeseries(cur_sub,ss_name,cur_ROI)
...
try
    ...
    % get rid of missings
    trial = trial(choice ~= 5);
    stim  = stim(choice ~= 5);
    cat   = cat(choice ~= 5);
   
    % output
    output = {trial, stim, cat, recorded_resp};
    
    % feedback
    pic_scr_feedback_m = [cur_sub ': Successful. ' success_message];
    disp (pic_scr_feedback_m)
    pic_scr_feedback = [pic_scr_feedback_m,output];
