% script to extract not first eigenvariate but the mean of the ROI per sub
function [Y,all_subs] = agk_get_secondlevel_ROI_mean(path_scnd_level_SPM,path_ROI,con)
...
%     end
    ...
    xY.name    = new_name_1;
    disp('Extracting now!')
    [Y, xY]    = agk_spm_regions(cur_xSPM,cur_SPM,[],[xY]);
    
    % now we do not take the first eigenvariate but replace it by mean
    Y          = mean(xY.y');
    Y          = Y';
    if any(isnan(Y))
        Y      = [];
    ...
    disp([path_scnd_level_SPM ': Something went wrong.']);
    Y = MExc;
